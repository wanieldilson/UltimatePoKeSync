...
--
-- Every entry is per game code, and that is the whole key: the Italian Ruby and Sapphire
-- keep their party sixteen bytes above the USA ones, so a localisation can move it where a
-- revision does not. Both Italian addresses are verified against live memory; the USA ones
-- come from the three sources of D-013 and have never been run. See D-035.
--------------------------------------------------------------------------------
...
local GAMES = {
	...
	-- Italian releases of the same four. FireRed and LeafGreen keep the USA addresses,
	-- confirmed by counting how often each appears as a literal inside the ROM
	-- (tools/check-gen3-addresses.py, D-034). Ruby and Sapphire do not: their party sits
	-- sixteen bytes higher, found by reading live memory and verified on both. See D-035.
	["AXVI"] = { name = "Ruby (Italy)",      gen = 3, party = 0x03004370, count = 0x03004360 },
	["AXPI"] = { name = "Sapphire (Italy)",  gen = 3, party = 0x03004370, count = 0x03004360 },
